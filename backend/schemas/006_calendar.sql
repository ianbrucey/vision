...
-- Date-range queries filter on start_time directly.
-- Expression index on (start_time::DATE) is deferred — the cast is not IMMUTABLE
-- due to timezone dependence. For per-case event volumes (<10K rows), the
-- regular btree index on start_time is sufficient.
